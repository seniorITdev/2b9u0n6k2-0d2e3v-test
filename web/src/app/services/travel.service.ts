...
export class TravelService {
  ...
  removeExpense() {
    ...
    const index = this.selectedExpenseIndex.getValue();
    const newExpenses = [
      ...currentExpenses.slice(0, index ?? currentExpenses.length),
      ...currentExpenses.slice((index ?? currentExpenses.length) + 1),
    ];
  ...
  setAddStatus() {
    ...
    if (selectedIndex !== null)
      currentExpenses[selectedIndex].status = ExpenseStatus.ACTIVE;
  ...
  setEditData(index: number) {
    ...
    if (index === selectedIndex) {
      currentExpenses[index].editStatus = 'EDITING';
      currentExpenses[index].deleteStatus = '';
    } else {
      if (selectedIndex !== null) {
        currentExpenses[selectedIndex].editStatus = '';
        currentExpenses[selectedIndex].deleteStatus = '';
      }
      currentExpenses[index].editStatus = 'EDITING';
      currentExpenses[index].deleteStatus = '';
    }
  ...
  setRemoveData(index: number) {
    ...
    const selectedIndex = this.selectedExpenseIndex.getValue();
    if (index === selectedIndex) {
      currentExpenses[index].deleteStatus = 'DELETING';
      currentExpenses[index].editStatus = '';
    } else {
      if (selectedIndex !== null) {
        currentExpenses[selectedIndex].deleteStatus = '';
        currentExpenses[selectedIndex].editStatus = '';
  ...
  markExpense(index: number) {
    const currentExpenses = this.expenseList.getValue();
    currentExpenses[index].status =
      currentExpenses[index].status === ExpenseStatus.DONE
